...
class VkInstanceCreateInfo(private val seg: MemorySegment) : IHeapObject(seg) {
    ...
    var layers: Array<String>
        get() = seg.get(ADDRESS, OFFSETS[5]).toCPointerArray(seg.get(JAVA_INT, OFFSETS[4])).map { it.fetchString() }
            .toTypedArray()
        set(value) {
    ...
    var extensions: Array<String>
        get() = seg.get(ADDRESS, OFFSETS[7]).toCPointerArray(seg.get(JAVA_INT, OFFSETS[6])).map { it.fetchString() }
            .toTypedArray()
        set(value) {
