...
namespace openminecraft::binary
{
    ...
    #define readbe16(target) read((char*) &target, sizeof(uint16_t)); be16(target)
    #define readbe32(target) read((char*) &target, sizeof(uint32_t)); be32(target)
    #define readbe64(target) read((char*) &target, sizeof(uint64_t)); be64(target)
    #define readbef(target) read((char*) &target, sizeof(float)); bef(target)
    #define readbed(target) read((char*) &target, sizeof(double)); bed(target)
